...
We can start from the **geneexp** object, that contains data from *expression_20genes.csv*: we will represent **WT1** on the x axis and **WT2** on the y axis.
...
1. Create a scatter plot of WT2 (x-axis) versus WT1 (y-axis).
...
```{r}
ggplot(data=geneexp, mapping=aes(x=WT2, y=WT1)) + 
  geom_point()
```
...
mybluescatterplot <- ggplot(data=geneexp, mapping=aes(x=WT1, y=WT2)) + 
  geom_point(color="blue", size=2, shape="square cross") +
  ggtitle(label="my second ggplot", subtitle="nice blue squares")
...
```{r}
ggplot(data=geneexp, mapping=aes(x=WT1, y=WT2, label=Gene)) + 
  geom_point() +
...
```{r, fig.width=7}
ggplot(data=geneexp, mapping=aes(x=WT1, y=WT2, label=Gene, shape=DE)) + 
  geom_point() +
...
```{r, fig.width=7}
ggplot(data=geneexp, mapping=aes(x=WT1, y=WT2, label=Gene, color=DE)) + 
  geom_point() +
  geom_text(nudge_x=1.2, size=3)
```
...
```{r}
ggplot(data=geneexp, mapping=aes(x=WT1, y=WT2, color=DE)) + 
  geom_point() +
...
```{r}
 ggplot(data=geneexp, mapping=aes(x=WT1, y=WT2, color=WT1)) + 
